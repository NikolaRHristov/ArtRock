...
impl Default for GridDimensions {
	fn default() -> Self { GridDimensions { width:32, height:32, depth:32 } }
}
...
impl Default for ScalarFieldShapeParams {
	fn default() -> Self {
		ScalarFieldShapeParams {
			// Sensible default
			base_sphere_radius:0.8,

			// Sensible default
			base_sphere_influence:1.0,

			sphere_distort_fbm:FbmParameters::default(),

			large_form_fbm:FbmParameters::default(),

			medium_detail_fbm:FbmParameters::default(),

			fine_detail_fbm:FbmParameters::default(),
		}
	}
}
...
pub fn generate_scalar_field(
	...
	scalar_field.par_iter_mut().enumerate().for_each(|(index, cell_value)| {
		...
		// sphere_distort_fbm.amplitude could be used here
